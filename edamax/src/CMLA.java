 ...
 */
public class CMLA extends Algorithm {
	...
	int tournament_size = 2; // the size of the tournaments
	 ...
	 */
	public CMLA(PropertyReader props, Problem problem, BufferedWriter out) {
	...
	/*
	 * private void updateProportional(int i, double f0, double f1) { double sol
	 * = t[i] (f1 t[i]) / (f0 (1 - t[i]) + f1 t[i]); t1[i] = alpha sol +
	 * (1-alpha) t[i]; }
	 * 
	 * private void updateDirect(int i, double f0, double f1) { double sol = (f1
	 * t[i]) / (f0 (1 - t[i]) + f1 t[i]); t1[i] = alpha sol + (1-alpha) t[i]; }
	 */
